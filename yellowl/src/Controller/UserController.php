...
class UserController extends AbstractController
{
    ...
    public function putUserPassword(int $id, Request $request, UsersRepository $usersRepo, EntityManagerInterface $em)
    {
        $data = json_decode($request->getContent(), true);
    ...
    public function putUserAvatar(int $id, Request $request, UsersRepository $usersRepo, EntityManagerInterface $em)
    {
        $data = json_decode($request->getContent(), true);
    ...
    #[Route('/auth/signin/{email}/{password}', name:'signin', methods:['GET'])]
    public function userSignin(string $email, string $password, UsersRepository $usersRepo)
    {
        $existingUser = $usersRepo->findOneBy([
            'emailAddress' => str_replace("%", ".", $email),
            'password' => $password,
        ]);

        if (empty($existingUser)) {
            return( new JsonResponse("User non trouvé"));
        } else {
            return $this->json($existingUser, 200, [], ["groups" => "users:read"]);
        }
    }
